...
<header class="header">
  ...
  <div class="header-right">
    ...
    <div class="header-info"><span id="hdr-weather"></span><span style="opacity:.4;padding:0 2px">|</span><span class="header-clock" id="hdr-clock"></span></div>
...
`)
	...
	sb.WriteString(`</div></div><script>if('serviceWorker' in navigator){navigator.serviceWorker.register('/sw.js').catch(function(){});}(function(){var cEl=document.getElementById('hdr-clock'),wEl=document.getElementById('hdr-weather');if(cEl){var fmt=new Intl.DateTimeFormat('zh-CN',{hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false,timeZone:'Asia/Shanghai'});function tick(){cEl.textContent=fmt.format(new Date())}tick();setInterval(tick,1000);}if(wEl){fetch('https://wttr.in/Beijing?format=j1').then(function(r){return r.json()}).then(function(d){var c=d.current_condition[0];var zh=c.lang_zh&&c.lang_zh[0];wEl.textContent='\u5317\u4eac '+(zh?zh.value:c.weatherDesc[0].value)+' '+c.temp_C+'\u00b0C'}).catch(function(){});}})();</script></body></html>`)
...
(function(){var cEl=document.getElementById('hdr-clock'),wEl=document.getElementById('hdr-weather');if(cEl){var fmt=new Intl.DateTimeFormat('zh-CN',{hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false,timeZone:'Asia/Shanghai'});function tick(){cEl.textContent=fmt.format(new Date())}tick();setInterval(tick,1000);}if(wEl){fetch('https://wttr.in/Beijing?format=j1').then(function(r){return r.json()}).then(function(d){var c=d.current_condition[0];var zh=c.lang_zh&&c.lang_zh[0];wEl.textContent='\u5317\u4eac '+(zh?zh.value:c.weatherDesc[0].value)+' '+c.temp_C+'\u00b0C'}).catch(function(){});}})();
</script>
</body></html>`,
...
<script>if('serviceWorker' in navigator){navigator.serviceWorker.register('/sw.js').catch(function(){});}(function(){var cEl=document.getElementById('hdr-clock'),wEl=document.getElementById('hdr-weather');if(cEl){var fmt=new Intl.DateTimeFormat('zh-CN',{hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false,timeZone:'Asia/Shanghai'});function tick(){cEl.textContent=fmt.format(new Date())}tick();setInterval(tick,1000);}if(wEl){fetch('https://wttr.in/Beijing?format=j1').then(function(r){return r.json()}).then(function(d){var c=d.current_condition[0];var zh=c.lang_zh&&c.lang_zh[0];wEl.textContent='\u5317\u4eac '+(zh?zh.value:c.weatherDesc[0].value)+' '+c.temp_C+'\u00b0C'}).catch(function(){});}})();</script>
</body></html>`)
	fmt.Fprint(w, sb.String())
}
